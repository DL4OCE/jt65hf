...
C      http://developer.berlios.de/projects/wsjt/.
     ...
     +  ftrack,nafc,mode65,s2,ical,wisfile)

C  Computes the spectrum for each of 126 symbols.
C  NB: At this point, istart, f0, and ftrack are supposedly known.
C  The JT65 signal has Sync bin + 2 guard bins + 64 data bins = 67
C  bins.  We add 5 extra bins at top and bottom for drift, making 77
C  bins in all.
...
C  Mix sync tone down to f=5*df (==> bin 6 of array cx, after FFT)
               ...
               call four2a(cx,nfft,1,-1,1,ical,wisfile)
               do i=1,77
