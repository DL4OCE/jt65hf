...
C      http://developer.berlios.de/projects/wsjt/.
     ...
     +  mode65,dtx,dfx,snrx,snrsync,ccfblue,ccfred1,flip,width,ical,
     +  wisfile)
...
C  Do FFTs of symbol length, stepped by half symbols.  Note that we
C  have already downsampled the data by factor of 2.
      nsym=126
      nfft=2048
      nsteps=2*jz/nfft - 1
      nh=nfft/2
...
!      fbmax=1870.46
      ...
      fa=famin
      fb=fbmax
C     [DEBUG] Implicitly setting NFreeze
      NFreeze=1
...
C  If we found nothing with snrx > -30 dB, take the best sync that
C  *was* found.
