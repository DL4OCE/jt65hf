...
begin
  //if AValue <> FForeColor then begin
    FForeColor := AValue;
    Refresh;
  //end;
end;
...
begin
  ...
  begin
    ...
    Font.Name := 'Arial';
    //Font:= Self.Font;
    if ShowPercentage then S    := format('%d%%', [PercentDone])
 	  else S:= Text;
...
begin
  if AValue <> FMaxValue then begin
    FMaxValue := AValue;
    Refresh;
  end;
end;
...
begin
  if AValue <> FShowText then begin
    FShowText := AValue;
    Refresh;
  end;
end;

procedure TColorProgress.SetText(AValue: String);
begin
...
procedure TColorProgress.SetShowPercentage(AValue: boolean);
begin
  if AValue <> FShowPercentage then begin
